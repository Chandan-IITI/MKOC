...
%%% Codes for MKOC_Thr1 i.e. MKOC_Theta1 as per paper
...
%%%%% Start Thr1 (Theta1 as per paper)
%%% For training
m = size(train_data,2);
labeltr = ones(m,1)*2;
difftr = abs(ones(m,1)-Otr');
[sout,~] = sort(difftr);
thresh = sout(ceil(m*(1-0.05)),1);
labeltr(difftr<thresh)=1;

%%% For Testing
labels = ones(size(test_data,2),1)*2;
diffs = abs(ones(size(test_data,2),1)-Ot');
labels(diffs<thresh)=1;
%%%%% End of Thr1 (Theta1 as per paper)
