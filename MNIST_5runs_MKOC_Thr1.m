...
%%% Following lines of code evaluate the method MKOC_Thr1 i.e MKOC_Theta1 as per paper 
...
%%%   Another way
    ...
    for i=1:length(Range_gamma)
        ...
        for j=1:length(Range_C)
            C = Range_C(j);     
                  ...
                  %%%% MKOC_Thr1
                  [labels_MKOC labeltr_MKOC] = MKOC_Thr1(train_data,val_data,layer,C,gama);   
    ...
    %%% Final training on optimal parameters
                  %%%% MKOC_Thr1
      [labels_MKOC, ~] = MKOC_Thr1(train_data, test_data, layer,C_opt,gama_opt); 
...
xlswrite('MNIST_results_Thr1.xlsx', gmean.*100,['digit' num2str(digit_num)],'B2');
...
xlswrite('MNIST_results_Thr1.xlsx', mgmean,['digit' num2str(digit_num)],'H2');

gmean_final = gmean.*100;
stdev_gmean = std(gmean_final,0,2);
xlswrite('MNIST_results_Thr1.xlsx', stdev_gmean,['digit' num2str(digit_num)],'J2');
